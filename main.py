...
<i>Это может занять 30-60 секунд...</i>
            ...
            try:
                ...
                # НОВАЯ ЛОГИКА: Создаем фабрику для BrowserManager (с тестовой конфигурацией)
                def browser_manager_factory(config, proxy_manager):
                    browser_config = {
                        'browser': {
                            'headless': False,  # Используем headless=False как в тестах
                            'window_size': '1920,1080',
                            'page_load_timeout': 30,  # Как в тестах
                            'implicit_wait': 10,
                            'user_agents': [
...
async def main() -> None:
    ...
    global config, bot, proxy_manager
